...
class Model_TrainTest:
    ...
    def train(self):
        ...
        for episode in range(1, self.max_episodes + 1):
            ...
            while not done and not truncation:
                action = self.agent.select_action(state, self.temperature)
    ...
    def plot_training(self, episode):
        ...
        if episode == self.max_episodes:
            plt.savefig('./lunarLander/boltzman/reward_plot.png', format='png', dpi=600, bbox_inches='tight')
        plt.tight_layout()
        plt.grid(True)
        plt.show()
        plt.clf()
        ...
        # Only save as file if last episode
        if episode == self.max_episodes:
            plt.savefig('./lunarLander/boltzman/Loss_plot.png', format='png', dpi=600, bbox_inches='tight')
        plt.tight_layout()
        plt.grid(True)
        plt.show()
        ...
        # Only save as file if last episode
        if episode == self.max_episodes:
            plt.savefig('./lunarLander/boltzman/Temperature.png', format='png', dpi=600, bbox_inches='tight')
        plt.tight_layout()
        plt.grid(True)
        plt.show()
...
if __name__ == '__main__':
    ...
    RL_hyperparams = {
        ...
        "RL_load_path": f'lunarLander/DQN/final_weights' + '_' + '1500' + '.pth',
        "save_path": f'./lunarLander/boltzman/final_weights',
        "save_interval": 500,
        "clip_grad_norm": 3,
        "learning_rate": 5e-4,
